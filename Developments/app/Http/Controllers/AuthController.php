...
{
    /**
     * Helper: tạo AccountID kế tiếp theo prefix (ADM/INS/LRN/ACC) cho MySQL 8.0.3
     * Dựa vào phần số ở cuối lớn nhất (REGEXP_SUBSTR) rồi +1, pad 3 chữ số.
     */
    private function nextAccountId(string $prefix): string
    {
        $last = DB::table('accounts')
            ->whereRaw("AccountID REGEXP CONCAT('^', ?, '[0-9]+$')", [$prefix])
            ->orderByRaw("CAST(REGEXP_SUBSTR(AccountID, '[0-9]+$') AS UNSIGNED) DESC")
            ->value('AccountID');
    ...
    {
        // Validate rõ ràng + unique cho AccountID (nếu gửi) & Email
        $validated = $request->validate([
            'AccountID' => 'nullable|string|max:50|unique:accounts,AccountID',
            'AName'     => 'required|min:2',
            'Email'     => 'required|email|unique:accounts,Email',
            'Pass'      => 'required|min:6',
            'ARole'     => 'required|in:Admin,Instructor,Learner',
            // tùy chọn, nếu không gửi sẽ set mặc định phía dưới
            'AStatus'        => 'sometimes|in:Active,Inactive,Banned',
            'ApprovalStatus' => 'sometimes|in:Approved,Pending,Rejected',
        ]);

        // Chọn prefix theo Role (giữ nguyên logic đặt tên)
        $role = $validated['ARole'];
        $prefix = match ($role) {
            'Learner'    => 'LRN',
            'Instructor' => 'INS',
            'Admin'      => 'ADM',
        ...
        // Nếu client gửi sẵn AccountID (đã qua unique) thì dùng, nếu không -> tự sinh theo MySQL 8 REGEXP_SUBSTR
        $accountId = $validated['AccountID'] ?? $this->nextAccountId($prefix);

        // Nếu không gửi AStatus/ApprovalStatus -> set mặc định như cũ
        $status   = $request->input('AStatus', 'Active');
        $approval = $request->input('ApprovalStatus', $role === 'Instructor' ? 'Pending' : 'Approved');

        // Tạo account (Pass vẫn plain để không phá vỡ login hiện tại)
        $account = Account::create([
            'AccountID'      => $accountId,
            'AName'          => $validated['AName'],
            'Email'          => $validated['Email'],
            'Pass'           => $validated['Pass'], // DEMO: plain; muốn hash sẽ cần sửa login() dùng Hash::check()
            'ARole'          => $role,
            'AStatus'        => $status,
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->filled('new_password')) {
            // DEMO: plain-text (sản phẩm thật hãy Hash::make và đổi login() dùng Hash::check)
